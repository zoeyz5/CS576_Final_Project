...
cfg = get_cfg()
cfg.merge_from_file(
    model_zoo.get_config_file("COCO-InstanceSegmentation/mask_rcnn_R_50_DC5_1x.yaml")
)
...
rotateCode = check_rotation("videos/" + video_name)
...
while success:
    ...
    all_mask = torch.zeros((height, width)).bool().to(torch.device(cfg.MODEL.DEVICE))
    for mask in masks:
    ...
    background = cv2.inpaint(
        background, cpu_mask.to(torch.uint8).unsqueeze(-1).numpy(), 3, cv2.INPAINT_TELEA
    )
